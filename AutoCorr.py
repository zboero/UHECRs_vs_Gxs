...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 24 14:40:33 2023
...
# -------
#
# This program computes the auto-correlation function of
# a galaxy sample.
#
# ------------------------------------------------------------------------------
...
def aCorr( th_rdm, phi_rdm, th_gxs, phi_gxs, bins ):
    ''' Radial profile around the position of galaxies
    Parameters
    ----------
    th_rdm : numpy array
    ...
    Returns
    -------
    A numpy array with the aCorrelation function
    
    '''
    prof_rdm = np.zeros( len(bins) - 1 )
    for i in range( len(th_rdm) ):
        prof_rdm_i = radial_profile( th_rdm[i], phi_rdm[i], th_gxs, phi_gxs, bins )
        prof_rdm  += prof_rdm_i
